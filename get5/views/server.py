...
def server_create():
    ...
    if request.method == 'POST':
        ...
        if max_servers >= 0 and num_servers >= max_servers and not g.user.admin:
            ...
            allow_offline = config_setting('ALLOW_OFFLINE_SERVERS')
            ...
            if mock or utils.check_server_connection(server) or allow_offline:
                db.session.commit()
                app.logger.info(
...
def server_edit(serverid):
    ...
    if request.method == 'POST':
        if form.validate():
            ...
            if mock or utils.check_server_connection(server):
                db.session.commit()
                return redirect('/servers')
            else:
                db.session.remove()
