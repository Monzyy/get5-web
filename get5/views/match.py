...
class MatchForm(Form):
    ...
    veto_mappool = MultiCheckboxField('Map pool',
                                      choices=[(
                                          name, utils.format_mapname(name)) for name in mapchoices],
                                      default=default_mapchoices,
                                      validators=[mappool_validator],
                                      )

    veto_process = StringField('Veto Process', default='',
                               validators=[
                                   validators.Optional(),
                                   validators.Regexp(r'^(\d:(b|p)\ ?)+$', message='Not valid syntax!')])
    veto_progress = StringField('Veto Progress', default='')
    ...
    if request.method == 'POST':
        ...
        if form.validate():
            ...
            if g.user.id != server.user_id:
                ...
                json_reply, message = utils.check_server_avaliability(
                    server)
            ...
            match = Match.create(
                ...
                form.data['veto_mappool'], form.data['veto_process'].split(), 
                server_id=server.id)
            ...
            if server_avaliable:
                ...
                flash(message + ". Go to match page and manually start match when server is available", 'warning')
...
def admintools_check(user, match, can_be_cancelled=False):
    ...
    grant_admin_access = user.admin and config_setting('ADMINS_ACCESS_ALL_MATCHES')
...
def match_edit(matchid):
    ...
    form = MatchForm(
        ...
        series_type="bo{}".format(match.max_maps) + ("-preset" if match.skip_veto else ""),
        team1=Team.query.get(match.team1_id),
        team2=Team.query.get(match.team2_id),
        veto_mappool=match.veto_mappool,
        veto_process=' '.join(match.veto_process),
        veto_progress=' '.join(match.veto_progress),
        )
    ...
    if request.method == 'GET':
        ...
        if form.validate(): # TODO form validation on edit
            skip_veto = 'preset' in form.data['series_type']
            try:
                max_maps = int(form.data['series_type'][2])
            except ValueError:
            ...
            update_dict = {
                ...
                'veto_process': form.data['veto_process'].split(),
                'veto_progress': form.data['veto_progress'].split(),
...
def match_start(matchid):
    ...
    json_reply, message = utils.check_server_avaliability(server)
    server_avaliable = (json_reply is not None)
    if server_avaliable:
...
def match_cancel(matchid):
    ...
    except (AttributeError, utils.RconError) as e:
