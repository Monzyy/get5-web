...
from ..utils import challonge, steamid
...
class TournamentForm(Form):
    ...
    veto_mappool = MultiCheckboxField('Map pool',
                                      choices=[(name, utils.format_mapname(name)) for name in mapchoices],
...
def admintools_check(user, tournament):
    ...
    grant_admin_access = user.admin and config_setting('ADMINS_ACCESS_ALL_TOURNAMENTS')
